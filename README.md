# Hi, I'm Berke! 👋

### Software Engineering Student at Samsun University

I am a first-year **Software Engineering** student focused on building a rock-solid foundation in computer science. My philosophy is to look beyond the surface—I don't just want to use tools; I want to understand the architectural decisions and system logic that make them scale.

---

### 🎯 Current Focus & Learning Path

*   **Python Mastery:** Deepening my knowledge in **Object-Oriented Programming (OOP)**, **Algorithms**, and **Data Structures**.
*   **Systems Thinking:** Learning **C++** to master low-level programming logic and efficient resource management.
*   **AI Exploration:** Actively experimenting with the latest **AI technologies** and LLM integrations to enhance software workflows.

---

### 🛠 Tech Stack

| Category | Skills & Interests |
| :--- | :--- |
| **Languages** | Python, C++ (Learning) |
| **Specialization** | System Logic, OOP, Data Structures & Algorithms |
| **Platforms** | Steam Ecosystem, Linux, Git/GitHub |
| **AI** | LLMs, RAG, Modern AI Tools |

---

### 📈 GitHub Statistics

![Berke's Stats](https://github-readme-stats.vercel.app/api?username=berkevnl&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=berkevnl&layout=compact&theme=tokyonight)

---

### 📫 Connect with Me

- **LinkedIn:** [linkedin.com/in/efeberkeaglik](https://linkedin.com/in/efeberkeaglik)
